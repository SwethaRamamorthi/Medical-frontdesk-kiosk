...
files.forEach(file => {
    ...
    // Bubble left to -4px to make it flush. We must be very precise.
    content = content.replace(/left: '20px', \n        marginBottom:/g, "left: '-4px',\n        marginBottom:");
    content = content.replace(/left: '20px',\n        marginBottom:/g, "left: '-4px',\n        marginBottom:");
    
    // Bot icon size
    content = content.replace(/fontSize: '3.5rem'/g, "fontSize: '3rem'");
    
    // Bot container position. Currently it's left: '20px', zIndex: 1000
